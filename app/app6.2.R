...
##//Yunyi ##transform weather
levels(collisionData$Weather)<-c(levels(collisionData$Weather),"Others")
collisionData$Weather[which(collisionData$Weather=='')]='Others'
collisionData$Weather[is.na(collisionData$Weather)]='Others'
...
##//Yunyi ##update weather levels
weatherSeq=c("Fog"=1,"Fog-Rain"=2,"Fog-Rain-Snow"=3,"Fog-Snow"=4,"Rain"=5,"Rain-Snow"=6,"Snow"=7,"Others"=8)
  ...
  tabItems(
           tabItem(tabName = "MapSetting",
                   ...
                   fluidRow(column(4,textInput("AddressInput","Find a Place:","Columbia University, NY"),
                            ...
                            column(4,checkboxGroupInput("Week", "Special Days:",daySeq),##//Yunyi
                                   hr(),
           ...
           tabItem(tabName = "SummaryByArea",
                   fluidRow(column(8,leafletOutput("Map2", width="100%",height=300)),
                            column(4,h3("Map Settings:"),
                                     h4("Months:"), 
                                     h5(textOutput("Month")), ##//Yunyi
                                     h4("Special Days:"), 
                                     h5(textOutput("WorkingDay")),##//Yunyi
                                     h4("Time Slot:"),
                                     h5(textOutput("TimeSlot")),##//Yunyi
                                     h4("Weather:"), 
                                     h5(textOutput("Weather")) ##//Yunyi
                                   )),
                   hr(),
...
###Page2  Summary By Area
  ...
  ##//Yunyi
  output$TimeSlot=renderText(paste("From",New.Sum.Para$New.TimeSlot[1],": 00","to",New.Sum.Para$New.TimeSlot[2],": 00")) 
  month=c("January","Febraury","March","April","May","June","July","August","September","October","November","December")
  output$Month=renderText(month[as.numeric(New.Sum.Para$New.Month)])
  weather=c("Fog ","Fog-Rain ","Fog-Rain-Snow ","Fog-Snow ","Rain ","Rain-Snow ","Snow ","Others ")
  output$Weather=renderText(weather[as.numeric(New.Sum.Para$New.Weather)])
  workingday=c("Weekdays","Weekends and Holidays")
  output$WorkingDay=renderText(workingday[as.numeric(New.Sum.Para$New.Week)])
