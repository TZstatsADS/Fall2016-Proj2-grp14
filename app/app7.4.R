  ...
  tabItems(
           tabItem(tabName = "SummarybyArea",
                   fluidRow(column(8,leafletOutput("Map2", width="100%",height=300)),
                            column(4,h4("Selected Area Summary:"),##//Yunyi
                                   h5("Total Number of Accidents:"), textOutput("AccidentNum"), 
                                   h5("Number of People Injured:"), textOutput("Injured"), 
                                     h5("Number of People Killed:"), textOutput("Killed"),
                                   h5("Vehicle Type with The Most Accidents:"),textOutput("Vehicle")
                                   )),
                   hr(),
...
###Page2  Summary By Area
  ...
  observeEvent(input$Map2_bounds,{  ##//  YOU
    ...
    New.Sum.Data2$ForSeasonality=subset(New.Sum.Data1$ForSeasonality, 
                                        LATITUDE  >= latRng[1] & LATITUDE  <= latRng[2] &
                                        LONGITUDE >= lngRng[1] & LONGITUDE <= lngRng[2] )
    New.Sum.Data2$ForContributingFactor=subset(New.Sum.Data1$ForContributingFactor, 
                                               LATITUDE  >= latRng[1] & LATITUDE  <= latRng[2] &
                                               LONGITUDE >= lngRng[1] & LONGITUDE <= lngRng[2] )
