...
sidebar <- dashboardSidebar(
  hr(),
  sidebarMenu(id="tabs",
              menuItem("Summary by Area", tabName = "SummarybyArea", icon = icon("area-chart")),
  ...
  tabItems(
           tabItem(tabName = "SummarybyArea",
                   fluidRow(column(8,leafletOutput("Map2", width="100%",height=300)),
                            column(4,h4(strong(id="TitleSummary","In this Area:")),##//Yunyi
                                   icon("random"),strong(" Total Number of Accidents: "), textOutput("AccidentNum"), br(),
                                   icon("bed"),strong(" Number of People Injured:"), textOutput("Injured"), br(),
                                   icon("bolt"),strong(" Number of People Killed:"), textOutput("Killed"),br(),
                                   icon("car"),strong(" Vehicle Type with The Most Accidents:"),textOutput("Vehicle")
                                  )),
                   hr(),
                   tabsetPanel(id="SummaryTabSet",
                              tabPanel("Map Setting",value="MapSetting",
                                       ...
                                       fluidRow(column(4,textInput("AddressInput","Find a Place:","Columbia University, NY"),
                                                       ...
                                                       sliderInput("AccdPt",label="# of Accidents Showed in the Map:",min=0,max=20000,value=500)),
                              ...
                              tabPanel("Casualty Forecast",value="TimeSeries",highchartOutput("TSPlot",width = "100%")),
                              tabPanel("Time Pattern",value="TimeAnalysis",fluidRow(column(4,plotlyOutput("TimebyYear")),
...
ui=dashboardPage(
  ...
  dashboardHeader(title = "Traffic Care"),
  sidebar,
  body
)
...
###Page2  Summary By Area
  ...
  summary_map=leaflet(collisionData[spr,])%>%
    ...
    addCircles(lat=~LATITUDE,lng = ~LONGITUDE,
               radius=~NUMBER.OF.PERSONS.INJURED+NUMBER.OF.PERSONS.KILLED+2,color="Red",
               layerId=~INDEX)%>%
  ...
  observeEvent(input$ApplyChanges,{  ##// YOU
    ...
    New.pointsNum=input$AccdPt
    spr1=sample(c(1:700000),size=New.pointsNum,replace=F)
    leafletProxy("Map2")%>%
      clearShapes()%>%
      addCircles(lat=~LATITUDE,lng = ~LONGITUDE,
                 radius=~NUMBER.OF.PERSONS.INJURED+NUMBER.OF.PERSONS.KILLED+2,color="Red",
                 layerId=~INDEX,data=collisionData[spr1,])
  ...
  observeEvent(input$Map2_bounds,{  ##//  YOU
    ...
    New.Sum.Data2$ForContributingFactor=subset(New.Sum.Data1$ForSeasonality, 
                                               LATITUDE  >= latRng[1] & LATITUDE  <= latRng[2] &
                                               LONGITUDE >= lngRng[1] & LONGITUDE <= lngRng[2] )
    New.Sum.Data2$ForTS=subset(New.Sum.Data1$ForTS, 
                               LATITUDE  >= latRng[1] & LATITUDE  <= latRng[2] &
                                 LONGITUDE >= lngRng[1] & LONGITUDE <= lngRng[2] )
    New.Sum.Data2$ForSummary=subset(New.Sum.Data1$ForSummary, 
                                    LATITUDE  >= latRng[1] & LATITUDE  <= latRng[2] &
  ...
  output$report <- downloadHandler(
    ...
    content = function(file) {
      ...
      params <- list(Map = DownloadMap, CtFactor=ContributingFactorDownload$p,
                     SeasonPlot=SeasonalityPlot$p,TSPlot=TSplots$TSPlot,
                     TimebyYear=TSplots$TimebyYear,TimebyMonth=TSplots$TimebyYear,
                     TimebyWeek=TSplots$TimebyWeek)
  ...
  output$ContributingFactor=renderPlotly({
    ...
    p <- plot_ly(
      ...
      add_lines(y=graphdata$Injured, name=paste("Number of","<br>","People Injured"),line=list(shape="linear"),color=I("black"),showlegend=TRUE) 
    ContributingFactorDownload$p=p
    p
  })
  ...
  observe({ 
    ...
    Death_Month = TrialForMonth$x
    salesForDay = Date_killed$x
    ...
    pforecast=hchart(airforecast)
    pyear=YearlyPlot(Date_killed)
    pmonth=MonthlyPlot(Date_killed)
    pweek=WeeklyPlot(Date_killed)
    
    TSplots$TSPlot=pforecast
    TSplots$TimebyYear=pyear
    TSplots$TimebyMonth=pmonth
    TSplots$TimebyWeek=pweek
    
    output$TSPlot=renderHighchart(pforecast)
    output$TimebyYear=renderPlotly(pyear)
    output$TimebyMonth=renderPlotly(pmonth)
    output$TimebyWeek=renderPlotly(pweek)
    }}
    })
...
###Page 5 Read Me
  output$ppplot3=renderText(paste("Urban mega cities face a myriad no of issues from population explosion to health. An explosion in the no of vehicles used has brought, with it a complex set of issues like collision. Collision compromises the safety and security of any urban agglomeration. Our application is aimed at helping policy makers to make informed decisions."))
  
###Page 6 About Team
  output$ppplot4=renderText("To be written")

###Page 7 To Be Developed
  output$ppplot5=renderText(" to show the link for cctv http://dotsignals.org/")
